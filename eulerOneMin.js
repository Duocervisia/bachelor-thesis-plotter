class EulerOneMin {
  run(ch) {
    ...
    for (let t = 0; t < nrSteps; t++) {
        let result = this.calcPV(t, ausrichtung, neigung, ch[t][0], ch[t][1], ch[t][2], ch[t][4], T_vor, true);
        let pv = result[0];
        T_vor = result[1];  
        results.push([ch[t][0], ch[t][4], T_vor]); 
    }
    return results;
  }
  calcPV(i, ausrichtung, neigung, t, G_h, G_dh, T_amb, T_vor, T_Vor_Anpassen) {
    ...
    let G_pv = result[0];
    //console.log(G_pv)
    result = this.calcPvDcBeyer(G_pv, T_amb, 60, 1, T_vor, T_Vor_Anpassen);
    let p_dc = result[0];
    T_vor = result[1];
...
Temperaturmodell(T_a, G, dt, T_vor, T_Vor_Anpassen) {
    ...
    if(T_vor === undefined){
        T_vor = T_a + c * G / 1000;
        T_pv = T_vor;
    }else if(T_Vor_Anpassen){
        let T_stat = T_a + c * G / 1000;
        T_pv = T_vor + (T_stat - T_vor) * (1 - Math.exp(-dt / tau));
        T_vor = T_pv;
    }else{
        T_pv = T_vor;
    }
